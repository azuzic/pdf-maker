<template>
    ...
    <div class="h-0.5 w-full bg-slate-400 rounded-full my-4"></div>
    <div class="text-xl font-bold text-slate-300 mb-2">Position:</div>
    <div class="flex flex-col gap-1 text-slate-300">
        <div class="flex gap-2">
            <div class="font-bold">Type: </div> 
            <AP_dropdowninput :label="positionType" @selected="updatePostion" :list="[
...
export default {
    ...
    data() {
        return {
            ...
            positionType: this.globalStore.selectedItem.positionType,
        }
    },
    async mounted() {
        ...
        this.positionType = this.globalStore.selectedItem.positionType;
    },
    methods: {
        updateWidth(value) {
            ...
            this.globalStore.selectedItem.classes = value.value;

            this.globalStore.selectedItem.widthType = this.widthType;
            this.globalStore.selectedItem.width = this.width;
            this.globalStore.selectedItem.positionType = this.positionType;

            let item = this.globalStore.PDFelements.find(item => item.id === this.globalStore.selectedItem.id);
        },
        updatePostion(value) {
        }
    }
