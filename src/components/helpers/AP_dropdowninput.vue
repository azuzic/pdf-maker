<template>
    <div class="w-full sm:max-w-sm flex gap-2 items-center">
        <div class="relative z-20 w-full group">
            ...
            :class="show ? 'text-slate-400' : 'text-slate-200'"/>

            <i class="fa-solid fa-caret-up w-4 h-4 absolute top-1.5 right-2 text-slate-400 peer-focus:text-slate-200 transition-all duration-300 z-10 pointer-events-none"
            ...
            <div @mouseleave="focus = false" @mouseenter="focus = true" :class="show ? 'h-fit max-h-64 opacity-100' : 'h-0 opacity-0'"
                class="w-full mt-2 | bg-slate-50 bg-opacity-5 | drop-shadow-md rounded | overflow-hidden | transition-opacity duration-300 | absolute overflow-y-auto">
                <ul class="text-sm z-50" aria-labelledby="dropdownDefaultButton">
                    <div v-for="v, id in  list " @click="value = v; show = false; $emit('selected', v)" 
                        class="block px-4 py-1 bg-slate-950 hover:bg-opacity-100 bg-opacity-95 cursor-default hover:font-bold z-50"
                        :class="v.value == value.value ? 'bg-AP_AccentFont text-emerald-500 font-bold' : 'text-slate-200'"> 
                        {{ v.label }} 
                    </div>   
                </ul>
            </div>
